...
```
sed -i '/#access_log */ s/logs\/access.log  main/syslog:server=192.168.100.141,facility=local2,tag=nginx_access,severity=info combined/' /etc/nginx.conf
sed -e '/nginx_access/ s/#access_log/access_log/' /etc/nginx.conf
```
